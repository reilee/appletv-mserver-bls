...
def downloadSubs(fpath, lang):
    ...
    for server in ["svplayer", "www","splayer1", "splayer2", "splayer3", "splayer4", "splayer5", "splayer6", "splayer7", "splayer8", "splayer9"]:
...
class SubFile(object):
	...
	try:
		uni=unicode(self.FileData,"GBK")
		self.FileData=uni.encode("UTF-8")
	except:
		pass
        if self.FileData.startswith("\xff\xfe"):
            self.FileData = unicode(self.FileData,"UTF-16").encode("UTF-8")
...
def getSub(fpath, languagesearch, languageshort, languagelong, subtitles_list):
    ...
    if (subdata):
        ...
        for sub in package.SubPackages:
            for fl in sub.Files:
                if (fl.ExtName in ["srt", "txt", "ssa", "smi", "sub","ass"]):
                    filename = ".".join([barename,'shooter', fl.ExtName])
                    subfilename = os.path.join(os.path.dirname(fpath),filename)
                    f=file(subfilename,"w")
                    f.write(fl.FileData)
                    f.close()
...
lis=[]
fname=sys.argv[1]
getSub(fname,"chn",'cn',"Chinese",lis)
#print lis
